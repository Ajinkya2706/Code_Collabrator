...
export function HeroSection() {
    ...
    return (
        <div className="relative min-h-screen bg-white text-black dark:bg-black dark:text-white overflow-x-hidden">
            {/* Animated Background */}
            <div
                aria-hidden
                className="z-[2] absolute inset-0 pointer-events-none isolate opacity-50 contain-strict hidden lg:block">
                <div className="w-[35rem] h-[80rem] -translate-y-[350px] absolute left-0 top-0 -rotate-45 rounded-full bg-[radial-gradient(68.54%_68.72%_at_55.02%_31.46%,hsla(0,0%,85%,.08)_0,hsla(0,0%,55%,.02)_50%,hsla(0,0%,45%,0)_80%)] animate-pulse" />
                <div className="h-[80rem] absolute left-0 top-0 w-56 -rotate-45 rounded-full bg-[radial-gradient(50%_50%_at_50%_50%,hsla(0,0%,85%,.06)_0,hsla(0,0%,45%,.02)_80%,transparent_100%)] [translate:5%_-50%] animate-pulse" />
                <div className="h-[80rem] -translate-y-[350px] absolute left-0 top-0 w-56 -rotate-45 bg-[radial-gradient(50%_50%_at_50%_50%,hsla(0,0%,85%,.04)_0,hsla(0,0%,45%,.02)_80%,transparent_100%)] animate-pulse" />
            </div>
            {/* Header */}
            <header className="fixed top-0 left-0 w-full z-30 bg-black/80 backdrop-blur border-b border-white/10">
                <div className="max-w-7xl mx-auto flex items-center justify-between px-8 py-4">
                    <div className="flex items-center gap-2">
                        <span className="text-xl font-bold flex items-center gap-2">
                            <span className="inline-block w-6 h-6 bg-gradient-to-tr from-blue-400 to-teal-400 rounded-md" />
                            tailark
                        </span>
                    </div>
                    <nav className="hidden md:flex gap-10 text-gray-300 text-base">
                        <Link href="#features" className="hover:text-white transition">Features</Link>
                        <Link href="#solution" className="hover:text-white transition">Solution</Link>
                    ...
                    <div className="flex gap-2 items-center">
                        <button
                            ...
                            onClick={toggleTheme}
                            className="p-2 rounded-full border border-white/20 bg-black/60 hover:bg-white/10 transition flex items-center justify-center"
                        >
                            {theme === 'dark' ? <Sun className="w-5 h-5 text-yellow-400" /> : <Moon className="w-5 h-5 text-blue-400" />}
                        </button>
                        <Link href="/login">
                            <button className="px-4 py-2 rounded-md border border-white/20 bg-black/60 hover:bg-white/10 transition">Login</button>
                        </Link>
                        <Link href="/register">
                            <button className="px-4 py-2 rounded-md bg-white text-black font-semibold hover:bg-gray-200 transition">Sign Up</button>
                        </Link>
                    </div>
                </div>
            </header>

            {/* Hero Content with scroll-based animation */}
            <main className="flex flex-col items-center justify-center min-h-screen pt-32">
                <div className="flex flex-1 flex-col items-center justify-center w-full h-full">
                    ...
                    >
                        <motion.div variants={transitionVariants.item} className="mb-6 flex justify-center">
                            <Link href="/login" className="inline-flex items-center px-4 py-1 rounded-full bg-black/10 text-sm text-black dark:bg-white/10 dark:text-white/80 border border-black/10 dark:border-white/10 hover:bg-black/20 dark:hover:bg-white/20 transition">
                                Let's make Collabration Easy
                                <ArrowRight className="ml-2 w-4 h-4" />
                            </Link>
                        </motion.div>
                        <motion.h1
                            variants={transitionVariants.item}
                            className="text-5xl md:text-7xl font-bold text-center max-w-4xl mb-6 text-black dark:text-white"
                        >
                            Modern Solutions for Customer Engagement
                        </motion.h1>
                        <motion.p
                            variants={transitionVariants.item}
                            className="text-lg text-center text-black/80 dark:text-white/80 max-w-2xl mb-10"
                        >
                    Collaborative development reimagined — live coding, canvas drawing, and team calls in a single smooth workspace.                        </motion.p>
                        <motion.div
                            variants={transitionVariants.item}
                            className="flex flex-col md:flex-row gap-4 items-center justify-center"
                        >
                            <Link href="/login">
                                <button className="px-8 py-3 rounded-xl bg-white text-black font-semibold text-base shadow hover:bg-gray-200 transition">Start Building</button>
                            </Link>
                            <Link href="/register">
                                <button className="px-8 py-3 rounded-xl border border-white/20 bg-black/60 text-white font-semibold text-base hover:bg-white/10 transition">Request a demo</button>
                            </Link>
                        </motion.div>
                    </AnimatedGroup>
                </div>
